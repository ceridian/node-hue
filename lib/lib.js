...
module.exports = {
	...
	tables: function(db, callback){
		if(db){
			var d = typeof(db);
			if(d == 'function'){
				db('lib.js: tables(): db not defined', null);
			}else{
				l.configs(function(err, conf){
					var targ = conf.webhcat_host.value.current.ipAddr;
					var port = conf.webhcat_port.value;
			...
			callback('lib.js: tables(): db not defined', null);
		}
	},

	columns: function(body, callback){
		if(body){
			var d = typeof(body);
			if(d == 'function'){
				body('lib.js: columns(): body not defined', null);
			}else{
				var db = body.db;
				var table = body.tab;
				l.configs(function(err, conf){
					...
					var port = conf.webhcat_port.value;
					var user = conf.query_user.value;
					var cmd = "http://"+targ+":"+port+"/templeton/v1/ddl/database/"+db+"/table/"+table+"/?user.name="+user+"&format=extended";
					request.get({
						'url': cmd,
					}, function(err, response, body){
						if(err){
			...
			callback('lib.js: columns(): body not defined', null);
		}
	},

	hiveQuery: function(query, callback){
		if(query){
			var d = typeof(body);
			if(d == 'function'){
				query('lib.js: sqlQuery(): query not defined', null);
			}else{
					...
					l.configs(function(err, conf){
						...
						request({
							...
							form: {
								...
								'callback': "http://"+ip+":3000/callback/$jobId",
								//'callback': "https://"+ip+"/callback/$jobId",
							}
						}, function(err, response, body){
							if(err){
								callback(err, null);
			...
			callback('lib.js: sqlQuery(): query not defined', null);
		}
	},
	...
	jobs: function(callback){
		l.configs(function(err, conf){
			...
			var port = conf.webhcat_port.value;
			var user = conf.query_user.value;
			var cmd = "http://"+targ+":"+port+"/templeton/v1/jobs?user.name="+user+"&showall=true&fields=*";
			request.get({
				'url': cmd,
			}, function(err, response, body){
				if(err){
	...
	jobDetail: function(id, callback){
		if(id){
			var d = typeof(id);
			if(d == 'function'){
				id('lib.js: jobDetail(): id not defined', null);
			}else{
				l.configs(function(err, conf){
					var targ = conf.webhcat_host.value.current.ipAddr;
					var port = conf.webhcat_port.value;
					var user = conf.query_user.value;
					var cmd = "http://"+targ+":"+port+"/templeton/v1/jobs/"+id+"?user.name="+user;
					request.get({
						'url': cmd,
					}, function(err, response, body){
						if(err){
			...
			callback('lib.js: jobDetail(): db not defined', null);
		}
	},
	...
	dirList: function(dir, callback){
		if(dir){
			var d = typeof(dir);
			if(d == 'function'){
				id('lib.js: dirList(): dir not defined', null);
			}else{
				l.configs(function(err, conf){
					var targ = conf.webhdfs_host.value.current.ipAddr;
					var port = conf.webhdfs_port.value;
					var user = conf.query_user.value;
					var cmd = "http://"+targ+":"+port+"/webhdfs/v1/"+dir+"?op=LISTSTATUS"
					request.get({
						'url': cmd,
					}, function(err, response, body){
						if(err){
			...
			callback('lib.js: jobDetail(): dir not defined', null);
		}
	}
}
